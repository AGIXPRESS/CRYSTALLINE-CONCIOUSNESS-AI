...
# Import Metal frameworks using PyObjC
try:
    import objc
...
class MetalShaderManager:
    ...
    def create_buffer(self, data: np.ndarray, length: int = None) -> Optional[any]:
        ...
        try:
            ...
            # Create an empty buffer with the right size
            ...
            # Make sure data is contiguous and flatten if multi-dimensional
            ...
            # Convert to bytes for more reliable copying
            data_bytes = data_flat.tobytes()
            
            # Use ctypes to copy bytes with correct pointer types
            ctypes.memmove(buffer_contents, ctypes.c_char_p(data_bytes), min(length, len(data_bytes)))
